...
def fit(epochs, model, loss_func, opt, train_dl, valid_dl, one_cycle=None, train_metric=True):
    '''
    ...
    for epoch in range(epochs):
        ...
        model.eval()
        with torch.no_grad():
            val_loss, val_accuracy, _ ,f1= validate(model, valid_dl, loss_func)
            if train_metric:
                train_loss, train_accuracy, _, _, = validate(model, train_dl, loss_func)
            else:
...
def validate(model, dl, loss_func):
    ...
    f1=f1_score(predictions, y_true, average='macro')

    accuracy = np.mean((predictions == y_true))
    return mean_loss, accuracy, (y_true, predictions),f1